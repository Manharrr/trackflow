...
def _resolve_order_tenant(request):
...
class OrderListAPIView(APIView):
    ...
    def get(self, request):
        tenant = _resolve_order_tenant(request)
        if not tenant:
            return Response(
    ...
    def post(self, request):
        ...
        with schema_context(tenant.schema_name):
            ...
            try:
                ...
                notify_order_created.delay(tenant.schema_name, order.id)
            except Exception as err:
                logger.exception(
                    f"[Celery Publish Error] Failed to publish notify_order_created task. "
                    f"Schema: {tenant.schema_name} | OrderID: {order.id} | Error: {str(err)}"
                )
...
class OrderDetailAPIView(APIView):
    ...
    def _get_order(self, request, order_id, tenant):
        try:
    ...
    def get(self, request, order_id):
        tenant = _resolve_order_tenant(request)
        if not tenant:
            return Response({"detail": "Tenant context could not be resolved."}, status=status.HTTP_400_BAD_REQUEST)

        with schema_context(tenant.schema_name):
            order = self._get_order(request, order_id, tenant)
