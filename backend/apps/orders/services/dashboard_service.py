...
class DashboardService:
    ...
    def get_dashboard_metrics(user, tenant):
        ...
        Executes inside the resolved tenant schema context.
        """
        if not tenant or not hasattr(tenant, "schema_name"):
        ...
        if not getattr(user, "is_superuser", False):
            from apps.tenants.models import UserTenant
            if not UserTenant.objects.filter(user=user, tenant=tenant, is_active=True).exists():
                return {}
