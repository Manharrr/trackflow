...
def _resolve_employee_tenant(request):
    tenant = getattr(request, "tenant", None) or resolve_request_tenant(request)
    if not tenant or not isinstance(tenant, Client):
        return None
    request.tenant = tenant
...
class EmployeeListAPIView(APIView):
    ...
    def get(self, request):
        tenant = _resolve_employee_tenant(request)
        if not tenant:
            return Response(
                {"detail": "Tenant context could not be resolved."},
                status=status.HTTP_400_BAD_REQUEST,
        ...
        with schema_context(tenant.schema_name):
            ...
            queryset = EmployeeService.list_employees(tenant).annotate(
...
class ActivateEmployeeAPIView(APIView):
    ...
    def post(self, request, employee_id):
        ...
        with schema_context(tenant.schema_name):
            ...
            operator = Employee.objects.filter(user=request.user, tenant=tenant).first()
